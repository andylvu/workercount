...
while True:
    try:
        ...
        crop = screen_capture[860:1080,580:1140]
        ...
        gray_sub_area = cv2.cvtColor(crop, cv2.COLORBGR2GRAY)    
        time2 = time.time()
        print(time2, 'time2')
        ...
        result2 = cv2.matchTemplate(gray_sub_area, scv_pic, cv2.TM_CCOEFF_NORMED)
        loc2 = np.where(result2 >= threshold_scv)
        time7 = time.time()
        print(time7, 'time7')
        # Perform template matching for the scv_multi
        result3 = cv2.matchTemplate(gray_sub_area, scv_multi_pic, cv2.TM_CCOEFF_NORMED)
