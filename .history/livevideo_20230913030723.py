...
def on_click(x, y, button, pressed):
    global right_click_start_time, right_click

    if button == Button.right and pressed:
        right_click_start_time = time.time()
    elif button == Button.right and not pressed and right_click_start_time is not None:
        right_click_end_time = time.time()
        if right_click_end_time - right_click_start_time < 0.005:  # Adjust the threshold as needed
            right_click = True
            print("Single right click detected")
