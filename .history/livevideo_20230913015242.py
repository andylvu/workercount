...
threshold_minerals = 0.7
threshold_scv = 0.7
threshold_scv_multi = 0.6
...
while True:
    try:
        ...
        result1 = cv2.matchTemplate(gray_upper, minerals_pic, cv2.TM_CCOEFF_NORMED)
        loc1 = np.where(result1 >= threshold_minerals)
        time6 = time.time()
        print(time6, 'time6')
        # Perform template matching for the scv
        result2 = cv2.matchTemplate(gray_lowerleft, scv_pic, cv2.TM_CCOEFF_NORMED)
        loc2 = np.where(result2 >= threshold_scv)
        time7 = time.time()
        print(time7, 'time7')
        # Perform template matching for the scv_multi
        result3 = cv2.matchTemplate(gray_lower, scv_multi_pic, cv2.TM_CCOEFF_NORMED)
